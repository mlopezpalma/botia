...
# Añade este código temporal en app.py para depurar
@app.route('/debug/twilio')
def debug_twilio_config():
    config = {
        'ACCOUNT_SID': os.environ.get('TWILIO_ACCOUNT_SID', 'No configurado'),
        'AUTH_TOKEN': os.environ.get('TWILIO_AUTH_TOKEN', 'No configurado') != 'No configurado',
        'WHATSAPP_NUMBER': os.environ.get('TWILIO_WHATSAPP_NUMBER', 'No configurado'),
        'ENABLED': whatsapp_bot.enabled,
        'CLIENT_INITIALIZED': whatsapp_bot.client is not None
    }
...
# Inicialización de WhatsApp Bot
whatsapp_bot = None

def init_whatsapp_bot():
    """Inicializa el bot de WhatsApp con configuración adecuada"""
    global whatsapp_bot
    ...
    logger.info(f"Inicializando bot de WhatsApp. SID configurado: {'Sí' if twilio_sid else 'No'}")
    logger.info(f"Token configurado: {'Sí' if twilio_token else 'No'}")
    logger.info(f"Número configurado: {twilio_number if twilio_number else 'No'}")
    
    try:
        # Importar e inicializar WhatsApp Bot
        from whatsapp_integration import WhatsAppBot
        whatsapp_bot = WhatsAppBot(app)
        logger.info(f"Bot de WhatsApp inicializado. Habilitado: {whatsapp_bot.enabled}")
        return whatsapp_bot
    except Exception as e:
        logger.error(f"Error al inicializar bot de WhatsApp: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())
        return None
...
if __name__ == '__main__':
    ...
    whatsapp_bot = init_whatsapp_bot()
